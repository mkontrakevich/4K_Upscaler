...
def _persist_candidate_to_worker(
    job_id: str,
    path: Path,
    ...
    This is deliberately executed before human review/final completion so the
    generated bytes no longer depend on the lifetime of the ephemeral
    Container. It does not invoke the generation pipeline.
    """
    token = os.environ.get("MG4K_PROCESSOR_TOKEN", "").strip()
    if not token:
        raise RuntimeError("MG4K_PROCESSOR_TOKEN is missing; RESULT cannot be persisted safely.")

    attempts = max(1, RESULT_PERSIST_ATTEMPTS)
    last_error = ""
    for attempt in range(1, attempts + 1):
        _set_state(
            job_id,
            status="processing",
            stage="result_persist_r2" if attempt == 1 else "result_persist_retry",
            progress=progress_value,
            result_path=str(path),
            result_persisted=False,
            result_persist_attempts=attempt,
        ...
        try:
            ...
            _set_state(
                job_id,
                result_persisted=False,
                result_persist_warning=last_error[:1200],
                result_persist_attempts=attempt,
            )
            if attempt < attempts:
                time.sleep(max(0.25, RESULT_PERSIST_RETRY_SECONDS))

    raise RuntimeError(
        f"RESULT was generated but Worker/R2 persistence failed after {attempts} attempts: {last_error}"
    )
